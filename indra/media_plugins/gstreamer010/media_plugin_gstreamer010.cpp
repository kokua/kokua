...
MediaPluginGStreamer010::load()
{
	    ...
	    (vis_name && std::string(vis_name)!="none"))
	{
		if (vis_name)
		{
			mVisualizer = gst_element_factory_make (vis_name, "vis");
		}
		if (!mVisualizer)
		{
			mVisualizer = gst_element_factory_make ("libvisual_jess", "vis");
			if (!mVisualizer)
			{
				mVisualizer = gst_element_factory_make ("goom", "vis");
				if (!mVisualizer)
				{
					mVisualizer = gst_element_factory_make ("libvisual_lv_scope", "vis");
					if (!mVisualizer)
					{
...
MediaPluginGStreamer010::unload ()
{
	...
	if (mVisualizer)
	{
		gst_object_unref (GST_OBJECT (mVisualizer));
		mVisualizer = NULL;
	}
...
MediaPluginGStreamer010::startup()
{
	...
	if (!mDoneInit)
	{
		...
		// do it.
		gst_registry_fork_set_enabled(false);// Since 0.10.10
