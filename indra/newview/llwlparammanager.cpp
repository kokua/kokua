...
void LLWLParamManager::loadPresets(const std::string& file_name)
{
	...
	LL_DEBUGS("WindLight") << "Loading Default WindLight settings from " << path_name << LL_ENDL;
	...
	LL_DEBUGS("WindLight") << "Loading User WindLight settings from " << path_name2 << LL_ENDL;
...
void LLWLParamManager::loadPreset(const std::string & name,bool propagate)
{
	...
	LL_DEBUGS("WindLight") << "Loading WindLight sky setting from " << pathName << LL_ENDL;
	...
	if(!presetsXML)
	{
		...
		LL_DEBUGS("WindLight") << "Loading User WindLight sky setting from " << pathName << LL_ENDL;
		presetsXML.clear();
        presetsXML.open(pathName.c_str());
	}
