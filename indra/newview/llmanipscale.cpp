...
void LLManipScale::dragCorner( S32 x, S32 y )
{
		 ...
		 iter != mObjectSelection->end(); iter++)
	{
		...
		if(  cur->permModify() && cur->permMove() && !cur->isAvatar() )
		{
			...
			F32 cur_max_scale_factor = llmin( LLWorld::getInstance()->getRegionMaxPrimScale() / scale.mV[VX], LLWorld::getInstance()->getRegionMaxPrimScale() / scale.mV[VY], LLWorld::getInstance()->getRegionMaxPrimScale() / scale.mV[VZ] );
...
void LLManipScale::stretchFace( const LLVector3& drag_start_agent, const LLVector3& drag_delta_agent )
{
		 ...
		 iter != mObjectSelection->end(); iter++)
	{
		...
		if( cur->permModify() && cur->permMove() && !cur->isAvatar() )
		{
			...
			F32 desired_scale		= llclamp(selectNode->mSavedScale.mV[axis_index] + desired_delta_size, LLWorld::getInstance()->getRegionMinPrimScale(), LLWorld::getInstance()->getRegionMaxPrimScale());
...
F32		LLManipScale::partToMaxScale( S32 part, const LLBBox &bbox ) const
{
	...
	max_scale_factor = bbox_extents.magVec() * LLWorld::getInstance()->getRegionMaxPrimScale() / max_extent;
...
F32		LLManipScale::partToMinScale( S32 part, const LLBBox &bbox ) const
{
	LLVector3 bbox_extents = unitVectorToLocalBBoxExtent( partToUnitVector( part ), bbox );
	bbox_extents.abs();
	F32 min_extent = LLWorld::getInstance()->getRegionMaxPrimScale();
	for (U32 i = VX; i <= VZ; i++)
	{
