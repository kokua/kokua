...
/*Kokua: fixme		ensure_equals("9 secondlife slurl, region + coords", slurl.getSLURLString(), 
					  "http://maps.secondlife.com/secondlife/myregion/1/2/3");
...
/*Kokua: fixme
		ensure_equals("16 grid slurl, region + coords - type", slurl.getType(), LLSLURL::LOCATION);
...
/*Kokua: fixme
		ensure_equals("26 app", slurl.getType(), LLSLURL::APP);
		...
		ensure_equals("32 app", slurl.getType(), LLSLURL::APP);		
		...
		ensure_equals("37 grid2", "util.aditi.lindenlab.com", slurl.getGrid());		
*/
		LLGridManager::getInstance()->setGridChoice("foo.bar.com");			
		...
		ensure_equals("40 region" , "myregion", slurl.getRegion());
		ensure_equals("41 grid3", "util.agni.lindenlab.com", slurl.getGrid());
...
/*Kokua: fixme
		...
		ensure_equals("43 location", slurl.getType(), LLSLURL::LOCATION);
		ensure_equals("44 region" , "myregion", slurl.getRegion());
		ensure_equals("45 grid4", "util.aditi.lindenlab.com", slurl.getGrid());		
		
		LLGridManager::getInstance()->setGridChoice("my.grid.com");
		slurl = LLSLURL("https://my.grid.com/app/foo/bar?12345");
		ensure_equals("46 app", slurl.getType(), LLSLURL::APP);		
...
/*Kokua: fixme 		ensure_equals("login string", slurl.getLoginString(), "uri:my region&amp;1&amp;2&amp;3");
